...
| Components global registration (`<NPage>` etc.) | Fixed | Same real user's device run — this got past `webpack.config.cjs`/`typescript`/`type:module` and past `ns run android`'s full build+install+launch (first time the app has actually run on a physical device), then crashed at Vue's `setup()` with `ReferenceError: useDevice is not defined`. Two separate gaps surfaced by this: components had no registration mechanism at all in the native bundle (Nuxt's `addComponentsDir` only registers globally for Nuxt's *own* build, which the native path never goes through), and composables had no auto-import either (see next row). Fixed the component half: the generated `.nuxt-native/app.js` bootstrap now imports `<NPage>`/`<NActionBar>`/`<NTabs>` from a new `nuxt-native/runtime/*` subpath export and registers them globally via `createApp(...).component(...)`, mirroring what Nuxt's own auto-registration does — the same mechanism (Vue's app-level component registry), just invoked directly. Verified the module builds these subpath exports correctly with a real (non-stub) `nuxt-module-build build`; not yet re-verified on an actual device (needs a push + fresh `create` + real hardware, which the reporting user has and this sandbox doesn't). |
